...
//  print_r($path_items);
  ...
  $csv = fopen($argv[1] . '/glaser-inventory-isl12.csv', 'a');
  ...
  // [6] isl12_pid; from filename
  $pid_slug = str_replace('_MODS.xml', '', $filename);
  $data[] = $pid = str_replace('_', ':', $pid_slug);

  // [7] isl12_content_model; from directory
  $data[] = $path_items[3];

  // [8] isl12_page_count; from idcrudfp query
  if ($path_items[3] == 'bookCModel') {
    $lines = [];
    exec("drush idcrudfp --root=/var/www/islandora71/caltech/current/docroot --user=1 --is_member_of=${pid} --pid_file=${argv[1]}/${pid_slug}.pids");
